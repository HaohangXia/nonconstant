...
| ID | 契约 | 落点 |
|---|---|---|
| P3-C1 | ⭐ **判据必须登记「一过一失败」**:`demo_pass_exit` 须为 `0`,`demo_fail_exit` 须非 `0`,并给 `demo_report`。⛔ 未登记 ⇒ 拒绝启用 | `nonconstant.yml:gates[*]` · `.nonconstant/meta-gate.sh` |
| P3-C2 | ⭐⭐ **判据的 `impl` 不存在 ⇒ 退出码 `2`,比「判红」更严重**(协议 **C6**)。⛔ 绝不等同于「未配置 ⇒ 跳过」 | `.nonconstant/meta-gate.sh` |
| P3-C3 | ⭐ **报告名里的 hash 须在 git 历史中存在且是 `HEAD` 的祖先** —— ⛔ 不是「等于当前 HEAD」(那条恒假) | `.nonconstant/report.sh` |
| P3-C4 | ⭐⭐ **base 只从 `nonconstant.yml:base` 取;调用方给 base ⇒ 拒绝,退出码 `2`** | `nonconstant.yml:base` · `.nonconstant/gates.sh`(**A004** 取代 P1-C4) |
...
| 上游契约 | 出处 | 兑现 |
|---|---|---|
| §3 序 4 闸门可执行性 | `01-PLAN.md` | ⭐ `.nonconstant/meta-gate.sh` |
| §3 序 5 M3 报告绑 commit | `01-PLAN.md` | ⭐ `.nonconstant/report.sh` |
...
| B6 名额 | `CLAUDE.md` | 用掉 **#6 `.nonconstant/meta-gate.sh`**、**#7 `.nonconstant/report.sh`** ⇒ **7/8 指名**;⛔ **未动用预算外余量** |

## gate_results

⭐ 全部在 **HEAD = `cad8012`、工作树干净** 上求值;收尾工作树 **0 项**。
⭐ fixture 由 `cp nonconstant.yml` **动态复制**后变异,⛔ 仓库内零静态副本。

| # | 检 / 探针 | 期望 | 实测 |
|---|---|---|---|
| 1 | **元判据 绿检** —— 真源 `nonconstant.yml`(4 条判据均登记双向演示) | 0 | ⭐ **0** |
...
⇒ ⭐⭐ **元判据满足了自己**(硬前置 C):它是 `nonconstant.yml` 里的第 3 条判据,登记了自己的双向演示,并在 #1 中被自己检查通过;#2/#3 证明它**拦得下**一条常量判据。
...
| 2 | ⚠️ **`nonconstant.yml` 里两条新判据的 `demo_report` 仍是 `phase3-PENDING.md`** | 元判据对 `*PENDING*` 放行(源码内显式白名单)⇒ ⛔ **一条判据可以永远声称 PENDING 而不可核查**。⇒ **下一 phase 开工第一件事:改成 `reports/phase3-cad8012.md`**。⛔ 本轮不改 —— 改它要动 `nonconstant.yml`,而报告须单独提交 |
...
| 5 | ⚠️ **`nonconstant.yml:base` 恒为 `HEAD`** | 「基线可配」这一能力**从未在非 `HEAD` 取值上测过** |
| 6 | ⚠️ **删闸仍返回 shell 的 127** | ⭐ 但本轮起有**两道独立覆盖**:① 元判据查 `impl` 存在性 ⇒ **2**;② `criteria-guard` 把删除视为对受保护路径的改动 ⇒ **1**(#10 实测)。⇒ ⛔ 编排层**仍须**显式处理 127(协议 C6),⛔ 不得依赖这两道兜底 |
| 7 | ⚠️ **未接 spec-kit** | 四条判据均已验证非交互、以退出码作答(**P3-C5**),⛔ 但 `workflows/nonconstant/workflow.yml`(B6 #8)本轮未建 —— 那是 **Q17**,§10 未列 |

## next_entry_conditions

1. ⭐ 本报告已提交,四条判据在 `cad8012` 内 —— **已满足**
2. ⬜ ⛔ **先把 `nonconstant.yml` 两处 `phase3-PENDING.md` 改为 `reports/phase3-cad8012.md`**(`known_gaps` #2)
...
- ⛔ 本轮建 `workflows/nonconstant/workflow.yml` 或任何编排层(Q17)

...
| ID | 契约 | 落点 |
|---|---|---|
| P2-C6 | ⭐ **扫描单位 = 函数作用域,⛔ 不用行数窗口** | `.latch/scan-silent.sh` 第二类判据 |
...
| P2-C10 | 新闸 `silent-scan` 登记于 `latch.yml`,`level: hard` | `latch.yml:gates[1]` |
...
| 2 | ⛔ **扫描器不区分「代码」与「字符串字面量」** | 自扫 `.latch/` 出 2 条,**全为假阳性** —— 是 `scan-silent.sh` 自己 `printf` 里的模式文本(`:44` `:45`)。注释行已由 **P2-C9** 排除(假阳性 6 → 2),⛔ 但字符串字面量需要真解析器。⇒ ⚠️ **当前若把 `silent-scan` 接到 `.latch/` 上,它会对自己判红** |

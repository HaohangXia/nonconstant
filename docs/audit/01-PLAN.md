...
## §10 已完成阶段(⭐ 档案,⛔ 不计入行数预算 —— A001 的 scope 模型)

> ⭐⭐ **节号 §10 一个字都没动** —— 22 处跨文件的「§10 Phase N」引用因此**全部仍然有效**(C11)。
> ⭐ 搬走的是**唯一未完成**的那一块(Phase 9+ → §11),⛔ 不是搬走已完成的十一块。
> ⛔ **判别是可核的,⛔ 不靠人挑**:「已完成」⇔ `reports/` 里有对应的 `phase<N>-*.md`。

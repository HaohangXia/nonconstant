# 同类工作扫描 —— ⛔ nonconstant 的差异化不成立（2026-08-23）

> ⛔⛔ **本文是一次证伪。**上一轮我列了四条「nonconstant 有、别人没有」的差异化，
> ⭐ 本轮逐条去核实 —— **四条全部落空**。
> ⚠️ 上限 60 行（`nonconstant.yml:doc_budgets`）。⛔ 发布仍停。
...
| # | nonconstant 以为独有 | `ryangu00/axiom` 的对应物 |
|---|---|---|
...
⇒ ⛔ **它主动承认竞品在某一维度上更强。**nonconstant 至今没做过这件事。

## ⛔ 这个空间比预想拥挤得多

⭐ axiom 的 `Prior art & related work` 一节点名**六个**同域项目，且各附访问日期：

| 项目 | 做什么 | ⚠️ 相对 nonconstant |
|---|---|---|
...
⚠️ ⛔ **nonconstant 的分发形态最弱**：axiom 有 4 个运行时适配（Claude Code · Codex CLI · hermes-agent · OpenClaw），nonconstant 只有一个 shell 安装器。

## ⚠️ 一处可能仍成立的差异（⛔ 未穷尽，按 F3 标注）

⭐ **它们判的是「agent 的声明」；nonconstant 有一层判的是「判据系统自身」**：
...
⇒ ⭐ 别人做「**验证 agent**」，nonconstant 有一部分在做「**验证你的验证**」。
...
⭐ **nonconstant 的差异化，按上一轮列的四条，不成立。**⛔ 且它在这个空间里**落后**：

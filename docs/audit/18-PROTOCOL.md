...
| C12 | ⚠️ 缺口(部分) | ⭐⭐ 以**匹配**取被判对象的判据,须同时防两形态:①「匹配失败」被当成「对象不存在」;⛔② **匹配成功但取值失效**(行尾注释/引号被吞进模式 ⇒ 恒不命中)。⛔ 两者效果相同:覆盖静默归零而判据全绿。⚠️ 仅 `status-facts` 已机器化 | `LATCH-pattern-miss-reports-pass` |
| C13 | ⚠️ 缺口 | ⭐⭐ **判据的启用条件,不得由被判对象提供** —— 否则一次编辑可同时完成「越界」与「让越界不可见」 | `LATCH-self-blinding` |
...
## §5 ⛔ 已撤回的约束(⛔ 不要重新施加)—— ⭐ 本节不计入预算(A001 scope);⛔ 但其长度是**撤回频次的指标**:增长过快 = 约束在被反复错误施加

...
| `LATCH-directive-self-audit-3`<br>下达方本轮自审:三处错<br>(2026-08-22) | — | ① 引用 `LATCH-budget-eats-the-fix` 论证「**绕开预算**」—— ⛔ 该条的正确结论是**改预算规则**(把「记录自身缺陷」排除在计数外),⛔ 不是绕开预算。**用一条规矩论证了一个规避动作。**<br>② 「余量 105 不是理由」**方向反了** —— 该句立意是「规矩不因有空间而不执行」,⛔ 却被用来支持「少记录」,与立意相反。<br>③ 「首次由执行方自发提出」未经核查(见 `LATCH-ledger-extraction-gap`) | **存在,三处。**⚠️ 判定者:下达方**自审** | — | ⚠️ ⭐ **下达方自审有效,⛔ 但仅在被外部要求时触发** —— 本轮三处错是在用户指出「没跑验证」之后才找到的。⇒ ⭐⭐ **与 `LATCH-orchestration-boundary` 第 ② 条实证同源、互相印证:自审有效但从不自触发 ⇒ 自动 loop 里没有人提出那个要求。** |
...
| ⭐⭐ `LATCH-self-referential-deadlock`<br>自律型约束阻止记录自身缺陷<br>(2026-08-22) | — | ⭐ **400 行上限正在阻止记录「400 行上限有问题」这件事** —— Q15 的完整论证写不进去,只能压成一行 | **存在。** | **推导:**约束阻止记录自身缺陷 ⇒ **不可自我修正** ⇒ **必然固化错误**(与 T5 同形:失败无输出 ⇒ 无人修 ⇒ 累积) | ⭐⭐ **任何自律型约束,必须允许「记录自身缺陷」的例外通道。**⚠️ §8「只增不删」同为自律型,**同样需要**。⛔ 例外通道的设计归 Phase 1 后的 amendment |
| `LATCH-proxy-criterion`<br>只想让数字变小,没问数字在测什么<br>(2026-08-22) | — | latch 侧下达「精简 §8、搬去 LEDGER」的指令,⛔ **未先问「400 这个数在测什么」**就去想办法让数字变小。⭐ 信息**一字未少**,只是换了账本 ⇒ ⛔ **不是精简,是藏账** | **存在。**⚠️ 判定者:latch 侧**自审**(⛔ 非执行方拦下) | ⛔ 代用品达标而被代用的性质未改善 ⇒ **Goodhart** | ⭐ **一般化:凡「某个数超了」,先问这个数在测什么。**⚠️ 与 DevLoop 侧「凡『根因是 X』的断言,先问它能否解释量到的分布」**同族** —— 两者都是「先验判据本身,再执行判据」 |

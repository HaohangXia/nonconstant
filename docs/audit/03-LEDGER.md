...
| `LATCH-incomplete-set-9`<br>行数分区枚举不完整<br>(2026-08-22,A001) | — | Q15 点名「现行结论 §1/§3/§5/§7/§10 · 档案 §2/§8/§9」,⛔ **§4 缺陷台账与 §6 未决问题两节未分类**。执行方按**分类判据**(这一节是不是每次都读)而非字面枚举处置,判为现行结论 ⇒ **293**(字面枚举则为 252) | ⚠️ **`LATCH-incomplete-set` 同族第 9 次**,由执行方拦下 | ⛔ 照字面执行会少算 36 行,且下次新增章节又要重吵 | ⭐ 用户裁定 **293 对**。⇒ 已在 §7a **就地写死分区规则**:档案仅 §2/§8/§9,其余一律计入,新增章节默认计入,除非在 A00x 中显式划为档案 |
...
| `LATCH-constant-criterion-inverse`<br>命名判据提交后恒假<br>(2026-08-22,`418ac92`) | — | latch 侧下达的 §10 Phase 3 命名判据 `ls "reports/phase$N-$(git rev-parse --short HEAD).md"` **以当前 HEAD 求值** ⇒ 报告一旦提交、HEAD 前移即失配 ⇒ **提交后恒假**。由执行方在 **Phase 0 收尾时撞破**(报告已写未提交的瞬间它为真,提交后就不再为真) | ⚠️ 与 `LATCH-constant-criterion` **同族但方向相反**:上次是恒真(`wc` 退出码恒 0),这次是恒假 | ⛔ 两者都是常量 ⇒ 都不是判据 | ⭐ **判定者独立性第六次生效。**修法(第一性):报告名 = **它所描述的那个 commit**,⛔ 非当下 HEAD ⇒ 判据改为「报告名里的 hash 在 git 历史中存在且是 HEAD 的祖先」,求值时机从「当下」变为「任何时候」。实测 绿 `0` / 红 `128` |

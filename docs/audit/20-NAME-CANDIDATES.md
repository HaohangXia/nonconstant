...
|---|---|---|---|---|---|---|
| **recuse** | 法官须**回避**自己是当事人的案子 = T1 的法律表述 | ⭐ 404 | ⭐ 404 | 2★ `xzycd/recuse`（**Polymarket 预言机争议 CLI**，TypeScript，已核） | ⭐⭐ **否，异域** | ⭐⭐ **可用** |
...
| ~~invigil~~ | 监考 | ⛔ **v1.7.1** | 404 | 1★ `invigil/invigil` | ⛔⛔ **是** | ⛔ **淘汰** |
| ~~crossexam~~ | 交叉盘问 | ⛔ **v0.6.1** | ⛔ 占用 | 6★ | ⛔ 是（多 AI 互审） | ⛔ 淘汰 |
| ~~hallmark~~ | 独立化验所打的纯度印记 | ⛔ v0.2.0 | ⛔ 占用 | **26639★** `Nutlope/hallmark` | ⚠️ 邻近 | ⛔ 淘汰 |
| ~~touchstone~~ | 试金石 | ⛔ v2.0.3 | ⛔ 占用 | 140★ | ⚠️ | ⛔ 淘汰 |
| ~~assay~~ | 独立测定 | ⛔ v0.0 | ⛔ 占用 | 126★ `mgattozzi/assay` | ⛔ 是（测试框架） | ⛔ 淘汰 |
| ~~arbiter~~ | 仲裁者 | ⛔ 占用 | ⛔ 占用 | 747★ `harnesslabs/arbiter` | ⚠️ | ⛔ 淘汰 |
| ~~iudex~~ | 拉丁「法官」 | ⛔ 占用 | ⛔ 占用 | 53★ `iudex/iudex` | ⚠️ | ⛔ 淘汰 |
| ~~deposition~~ | 证言笔录 | ⭐ 404 | ⛔ 占用 | 无 | ⛔ 是（法律 AI 一片） | ⛔ 淘汰 |
...
| 4 | corroborant | ⭐ 波普尔语义准（**经受住检验 ≠ 被证明**），⭐ 四处全清 | ⛔ 11 个字母、拗口 |
| 5 | disinterest | ⭐ 法律义精确 | ⛔⛔ 日常义（「不感兴趣」）会**反向误导** |
| 6 | nemojudex | 同 causasua | ⛔ 更长，⛔ 无优势 |

⇒ ⭐ **我推荐 `recuse`。**第一性：名字要承载**主张**，⛔ 不是承载**功能**。「闸门/检查」是功能（且已饱和）；⭐「判定者必须回避自己的案子」是主张，⛔ 目前无人占。

#!/usr/bin/env bash
#
# latch · upstream-semantics  —— 上游语义判据
#
#   ⭐ latch 只依赖 spec-kit 的**两条语义**(Q14 实测):
...
#
# 退出码:
#   0  过     —— 两条语义均与 latch 的依赖一致
#   1  未过   —— 语义已变 ⇒ ⛔ latch 的立身之本失效,须重新设计
#   2  闸自身故障 / **判不了** —— ⛔ 绝不因判不了而返回 0
#      ⚠️ 「装不上、导不进、结构对不上」一律判 2:本判据的职责是回答「语义还在吗」,
#         ⛔ 答不出 ≠ 通过(C6)。
#
# 用法:  bash .latch/upstream-semantics.sh [vendor 的 src 目录]
...
g = GateStep().execute({"message": "latch-probe"}, ctx)
...
if [ -n "$(printf '%s' "$BAD" | tr -d '[:space:]')" ]; then
  printf '⛔ upstream-semantics FAIL —— latch 依赖的上游语义已变:\n' >&2
  printf '%s' "$BAD" >&2
  printf '⇒ ⛔ 这不是「升级一下」的问题:shell 退出码与 gate 不自动放行是 latch 的立身之本。\n' >&2
  exit 1
fi

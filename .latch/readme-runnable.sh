#!/usr/bin/env bash
#
# latch · readme-runnable  —— README 里的命令必须真能跑
#
#   ⭐ 一份 README 最常见的失效方式:命令曾经能跑,后来文件改名了。
#      ⛔ 而 README 不会因此报错 —— 它只是**悄悄变成假的**(T5 形状)。
#
#   本判据查两件事:
#     ① README 的 ```bash 块里凡引用 `.latch/<x>.sh` / `install.sh` 的,
#        那个文件**必须存在**
#     ② ⭐ **三条必含内容必须在文中**(A008 §4)——
#        隐去任一条 = 把 latch 卖成「装上就不会错」
#
...
#   2  闸自身故障
#
# 用法:  bash .latch/readme-runnable.sh [README 路径]
...
DOC="${1:-}"
[ -n "$DOC" ] || DOC=$(latch_subject "$CONFIG" readme)
...
# ── ① 命令引用的文件必须存在 ──────────────────────────────────────
# 只取 ```bash 围栏内的行,再抽出形如 .latch/x.sh 或 install.sh 的 token
REFS=$(awk '
  /^```bash$/ { inb = 1; next }
  /^```/      { inb = 0 }
  inb         { print }
' "$DOC" | grep -o '\(\.latch/\)\?[A-Za-z0-9_-]\+\.sh' | sort -u)
...
[ "$NREF" -gt 0 ] || note "README 的 \`\`\`bash 块里一条 latch 命令都没有 —— ⛔ 一份没有可跑命令的 README 与没有 README 无法区分"

# ── ② 三条必含内容 ────────────────────────────────────────────────
# ⭐ 查的是**标记**,⛔ 不是措辞 —— 措辞会改,标记是契约(与 C12 第二形态同一教训:
#    ⛔ 别把判据绑在人类可读文案上)。README 里以 HTML 注释形式落这三个锚。
for anchor in latch:disclosure:mechanized-ratio \
              latch:disclosure:checkable-not-correct \
              latch:disclosure:known-gaps; do
  grep -qF "$anchor" "$DOC" \
    || note "README 缺少必含内容锚点:$anchor(A008 §4 —— 隐去 = 把 latch 卖成「装上就不会错」)"
done

...
nonconstant 把「检查」从**一句话**变成**一条命令 + 一个退出码**。

```
0  过        1  未过        2  ⛔ 闸自身故障 —— 判不了
```

⭐ 第三档是关键：**nonconstant 绝不因为「判不了」而返回 0。**扫描目标不存在、配置缺失、上游导不进来 —— 一律判 2，⛔ 而不是「没发现问题 ⇒ 放行」。
...
⭐ 这一条演示 nonconstant 的**核心主张**。全程约 30 秒，在一个全新空目录里跑：
...
装进来（`<nonconstant>` 换成你 clone 下来的 nonconstant 目录）：

```bash
bash <nonconstant>/install.sh .
...
```bash
sed -i 's|^  - .nonconstant/\*\*$|  - nothing/**|' nonconstant.yml
bash .nonconstant/gates.sh
```
...
```bash
git checkout -- nonconstant.yml
```

## 装完之后怎么用

安装器只装**可分发**的判据（`scope != bootstrap`），⛔ 不装 nonconstant 自举专用的那条。装完后有四处需要你填 —— ⭐ 每一处**没填时对应判据判 2 并指名缺什么**，⛔ 不会悄悄放行：
...
```bash
for g in .nonconstant/*.sh; do bash "$g" >/dev/null 2>&1; echo "$g=$?"; done
```

⭐ **想让它在提交时自动跑?**建一个 `.git/hooks/pre-commit`,里面调一个跑全部判据的脚本
(nonconstant 自己就这么做的,见 `.nonconstant/pre-commit.sh`)。⚠️ ⛔ 安装器**不装它** —— `.git/hooks/`
不受版本控制,装了也不会跟着仓库走。

⚠️ ⛔ 别用管道读退出码 —— 你读到的会是管道最后一个命令的。这个坑 nonconstant 自己踩过两次。
...
nonconstant 能测：**你说的和仓库里的是否一致**。
nonconstant ⛔ **测不了**：你说的那句话背后的**推理**对不对。

⚠️ 实例（本项目自己踩的）：一条判据声明「这个到期了」，判据如实判红 —— ⛔ 但它不会告诉你「你当初估的原因就是错的」。解开那个结的是一次证伪，**不是判据**。那一步仍然靠人。

<!-- nonconstant:disclosure:mechanized-ratio -->

⭐⭐ **nonconstant 自己的操作协议有 33 条，其中只有 5 条是机器化的。**

⛔ 其余 28 条要么是尚未实现的缺口，要么是**原理上不可判定**（比如「发现必须说」——机器判不了你有没有发现）。⇒ ⛔ 装上 nonconstant **不等于**那 33 条都被执行了。

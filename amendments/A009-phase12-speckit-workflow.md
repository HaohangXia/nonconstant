...
| 编号 | **A009** |
| 日期 | 2026-08-23 |
| 触发 | ⭐ 「引擎」被拆成**执行引擎 / 生成引擎**两类后（`LATCH-engine-two-kinds`），latch 缺的那一半变清楚了 |
| 状态 | ⭐ 已采纳，应用于 `docs/audit/01-PLAN.md` §11 |
| B6 名额 | ⭐ **用掉第 8 格** —— `workflows/latch/workflow.yml`，空了十二个 phase |
...
⚠️ ⛔ **本条不是 Q17（自动派单）。**⭐ 只做一件事：**让 workflow 调用 latch 判据，判红时真的停。**
...
| **latch** | ⭐ 十条判据 | ⛔ **时机** —— 靠手动敲，或 commit 时被动触发 |

### ⭐ 三条收益（⛔ Phase 12 须逐条确认或推翻）

| | 收益 | 实证依据 |
|---|---|---|
| a | ⭐ **判据需要固定触发点，否则退化成常量** | latch 自己：一条判据 **08-17 退化，08-20 才发现，中间那条闸跑了 0 次** |
...
**做**：`workflows/latch/workflow.yml`，用 spec-kit **内建** `shell` step（⛔ 不写自定义 step），显式处理 **127**。

**⭐ 跑在哪**：**装了 latch 的普通项目**（⛔ 不是 latch 自己的仓）。
理由：Phase 8 的教训 —— `config-read.sh` 没被装过去，⛔ 而那在 latch 自己的仓里**永远看不见**。
⚠️ 且 latch 自己的仓有 `upstream-pin`（`scope: bootstrap`），⛔ 用户项目里根本没有这条 ⇒ 在自己仓里测等于测一个没人有的布局。
...
**⭐ 收尾必答**：装了 latch 的项目里，用户的日常动作是什么？`gates.sh` / `specify workflow run` / `pre-commit` 若并存，说清什么时候用哪个。⛔ 答不出来说明 latch 还不能被用。

## 5 · 影响面

| # | 决定 | 出处 | 是否失效 |
|---|---|---|---|
| 1 | **B6 第 8 格**指名 `workflows/latch/workflow.yml` | `CLAUDE.md` B6 | ⭐ **本 phase 正是兑现它** —— ⛔ 不动名额规则 |
| 2 | **B1** 根目录不得新增其它目录 | `CLAUDE.md` B1 | ⚠️ ⛔ **须注意**：`workflows/` 是**新的根目录**。⭐ 但 B6 表**早已指名** `workflows/latch/workflow.yml` ⇒ 该目录是 Phase 0 就预期的，⛔ 非「新增其它目录」。⇒ ⚠️ B1 的表未列 `workflows/` ⇒ **表不完整**（与 A008 `known_gaps` #4 的 `README.md` 同形），⛔ 非违规 |
...
| 6 | `waiver-expiry` 用「已排最大 phase」 | `.latch/waiver-expiry.sh` | ⚠️ `MAXP` 11 → 12，⭐ 方向是**放宽**，⛔ 不产生新的红 |
...
| 2 | ⛔ **workflow 装不进用户项目** | `workflows/latch/workflow.yml` 在 latch 仓里，⛔ 而 `install.sh` 不装它（⭐ 与 `.git/hooks/` 同类问题）。⇒ ⚠️ Phase 12 须在报告里说清用户怎么得到它 |

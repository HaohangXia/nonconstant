...
⭐ **nonconstant 声称 pin 到 `v1.0.0` / `bca6790`,⛔ 而没有任何东西能证明磁盘那份就是它。**
...
| b' · 对照 | ⚠️ 用户**本来就需要** vendor 的 **15 MB / 639 个文件** 才能用 nonconstant ⇒ ⭐ **边际成本约 2 MB** |
...
| # | 缺口 | 说明 |
|---|---|---|
| 1 | ⚠️ **`.gitmodules` 算不算 B6 的一个文件?** | 它是 git 元数据(107 字节),⛔ 不是 nonconstant 的判据或配置。⭐ 本文判定**不计入** B6 —— 与 `.gitignore` 同类。⚠️ ⛔ 但 B6 的计数口径未写明「git 元数据不计入」⇒ 与 **Q12 / A002 `known_gaps` #2** 同族的口径缺口 |

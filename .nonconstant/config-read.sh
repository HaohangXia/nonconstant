#!/usr/bin/env bash
#
# nonconstant · config-read  —— ⭐⭐ 配置取值的**唯一**出口(⛔ 不可单独执行,只供 source)
#
...
#        · `- nonconstant.yml            # 配置本体`
#          ⇒ criteria-guard 的 glob 变成含注释的字符串 ⇒ 恒不命中
#          ⇒ `git status` 明明是 ` M nonconstant.yml`,而判据判 **0 PASS**(自遮蔽,C13)
...
nc_subject() { # nc_subject <配置文件> <键>
  nc_scrub "$(awk -v key="$2" '
    /^subjects:/ { inb = 1; next }
    /^[a-z_]/    { inb = 0 }
